...
@interface PolygonQueryViewController ()

@end

@implementation PolygonQueryViewController
@synthesize myPolygonQueryDelegate;
@synthesize BufferlblValue,Bufferpicklist,Segment,BoxPick,textBigClass,textSmallClass,btqueryBuffer,btnSmallclass,btnBigclass ,tbResultview,btnCreatBuffer,PopViewSon ,ResultView,CURleaf,ALLleaf,btnNextleaf,btnDrawBuffer,btnPreleaf;
...
AGSMutableMultipoint  *Mpoint3=nil;
...
- (IBAction)btnbufferquery:(UIButton *)sender {
    ...
    PopViewSon.hidden=false;
     [myPolygonQueryDelegate PopView ].hidden=PopViewSon .hidden;
    [[myPolygonQueryDelegate activityIndicatorView] startAnimating];
...
{
    [myPolygonQueryDelegate btnPolygonQueryshoworhide];
}
...
- (IBAction)BtnDrawBuffer:(UIButton *)sender {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(respondToGeomChangedPoly:) name:AGSSketchGraphicsLayerGeometryDidChangeNotification object:nil];//GeometryChanged时调用
	appDelegate.mapView.touchDelegate = sklayer;
	sklayer.geometry = [[AGSMutablePolygon alloc] initWithSpatialReference:appDelegate.mapView.spatialReference];
	...
	sklayer.mainSymbol=Symbol;    //NSLog(@"%@",sklayer.name);
    BufferlblValue.text=@"面积:";
}


- (void)respondToGeomChangedPoly: (NSNotification*) notification
{
	if (![sklayer.geometry isEmpty])
	{
		...
		if([geochange isKindOfClass:[AGSPolygon class]]){
			...
			if (area != 0) {
               ...
               BufferlblValue .text =[@"面积:" stringByAppendingString:[[NSString stringWithFormat:@"%0.3f", fabs(area)] stringByAppendingFormat: @" 平方米"]] ;
                }
                else
                {
                      BufferlblValue .text =[@"面积:" stringByAppendingString: [[NSString stringWithFormat:@"%0.3f", fabs(area)/1000000] stringByAppendingFormat: @" 平方公里"]];
                }
            }
...
- (IBAction)BtnCreateBuffer:(UIButton *)sender {
   ...
   // [graphicsLayer removeAllGraphics];
    ...
    if(geo==nil||sklayer.geometry.isEmpty)
    {
        ...
        [[myPolygonQueryDelegate   MainView] addSubview:dia.view];
...
//        [alert release];
    ...
    AGSGraphic *gr =[[AGSGraphic alloc]init   ];
    gr.geometry=sklayer.geometry;
    gr.symbol= FillSymbol  ;
...
{
    AGSGraphic *gra=[graphicsLayer.graphics objectAtIndex:0];
    AGSGeometry *geo=gra.geometry;
    AGSPolygon *s=(AGSPolygon *)geo;
    ...
    Mpoint3=[[AGSMutableMultipoint alloc] init];
    for (int i=0;i<[s numRings];i++)
    {
        for(int j=0;j< [s numPointsInRing:i];j++)
        {
            
            [Mpoint3 addPoint: [s pointOnRing:i atIndex:j]];
           pointed=[s pointOnRing:i atIndex:j];
     ...
     //传输多变形需要构造闭合点串
     pointed=[s pointOnRing:0 atIndex:0];
    ...
    return [NSString  stringWithFormat:@"POLYGON ((%@))",all] ;
}
...
-(void)GetBufferServiceBy:(NSString *)GEOMETRYstr
{
    ...
    NSString *str = [NSString stringWithFormat:@"GXTypes=%@&GeometryWKT=%@&f=%@",postLayernamestr,GEOMETRYstr,@"json"];
...
//        [alert release];
        ...
        PopViewSon.hidden=true;
         [myPolygonQueryDelegate PopView ].hidden=PopViewSon .hidden;
         [[myPolygonQueryDelegate activityIndicatorView] stopAnimating];
...
-(void)GetBufferServiceResult:(NSString *)GEOMETRYstr
{
    ...
    switch ( Segment.selectedSegmentIndex) {
            //管点
        case 0:
            str = [NSString stringWithFormat:@"GXName=%@&OutFields=%@&GeometryWKT=%@&PageNowAndSize=%@&f=%@&OutGeometry=%@",bs,@"OBJECTID,DMGC,TZD,FSW,SZDL,SSQY,QSDW,TCDW,TCRQ,JGCZ,JGZJ,JGC,JGK,JGXZ,JXJCZ,JBS,JSZJ",GEOMETRYstr,sendleaf,@"json",@"T"];
            break;
            //管线
        case 1:
            str = [NSString stringWithFormat:@"GXName=%@&OutFields=%@&GeometryWKT=%@&PageNowAndSize=%@&f=%@&OutGeometry=%@",bs,@"OBJECTID,CZ,GJ,SZDL,SSQY,QSDW,JSND,DYZ,YL,ZKS,ZYKS,DLTS,QDGC,ZDGC,QDMS,ZDMS,MSLX,TCDW,TCRQ",GEOMETRYstr,sendleaf,@"json",@"T"];
            
            break;
            //井室线
        case 2:
            str = [NSString stringWithFormat:@"GXName=%@&OutFields=%@&GeometryWKT=%@&PageNowAndSize=%@&f=%@&OutGeometry=%@",bs,@"OBJECTID,SSQY,XX,QDDH,ZDDH",GEOMETRYstr,sendleaf,@"json",@"T"];
            break;
        default:
            break;
    }
