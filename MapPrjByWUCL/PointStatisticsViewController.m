//
//  PointStatisticsViewController.m
//  NJSZGX（new）
//
...
@interface PointStatisticsViewController ()

@end

@implementation PointStatisticsViewController
@synthesize myPointStatisticsDelegate,ClassPick,ResultTable,TextBigclass,TextSmallclass ,ViewOFPick,TextRange,TextSon ,BtnselectValue,Alllayerstrarr,StatisitcsPopView,StatisticResultBox,activityIndicatorView;
int LoadPointFlag=0;//标识pickview绑定对应数据源
...
- (IBAction)btnCloseclick:(UIButton *)sender {
    ...
    [myPointStatisticsDelegate PopView].hidden=true;
...
-(void)InIlist
{
    ...
    [  StatisticsRangeName addObject:@"特征点"];
    [  StatisticsRangeName addObject:@"附属物"];
    ...
    [  StatisticsRangeField addObject:@"TZD"];
    [  StatisticsRangeField addObject:@"FSW"];
...
-(void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    if([errarr count]!=0)
    {
        ...
        [[myPointStatisticsDelegate   MainView] addSubview:dia.view];
...
- (UITableViewCell *)tableView:(UITableView *)tableView
    ...
    static NSString *TableSampleIdentifier = @"PointStatisticsCell";
    
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:
                             TableSampleIdentifier];
    if (cell == nil)
    {
        NSArray *nibArray = [[NSBundle mainBundle] loadNibNamed:@"PointStatisticsCell" owner:self options:nil];
    ...
    switch (sender.tag) {
        ...
        case 23://范围选择
            ...
            if(valueindex!=-1)
            {
                ...
                [self GetSonList:RangeMark LayerIndex:(valueindex+1)];
            }
            break;
...
-(void)GetStatisticsTask:(NSString *)GXType Field:(NSString *)field where:(NSString *)wherestr
{
    ...
    [request setHTTPMethod:@"POST"];
    //NSString *ClassName= [TextBigclass.text stringByAppendingString:[NSString stringWithFormat: @"_%@",TextSmallclass.text]];
    NSString *str = [NSString stringWithFormat:@"GXTypes=%@&LayerType=%@&WhereClause=%@&GroupBy=%@&f=%@",GXType,@"0",wherestr,field,@"json"];
    NSData *data = [str dataUsingEncoding:NSUTF8StringEncoding];
    [request setHTTPBody:data];
    //第三步，连接服务器
    NSURLConnection *connection = [[NSURLConnection alloc]initWithRequest:request delegate:self];
